...
SpriteTable::SpriteTable(std::shared_ptr<IRenderer> renderer, IMemory* oam)
	: _renderer(renderer), _oam(oam)
{
	...
	for (int fine_y = 0; fine_y < 8; ++fine_y)
	{
		for (int fine_x = 0; fine_x < 8; ++fine_x)
		{
			...
			case 3:
				...
				break;
			default:
				image[image_index] = {0xff, 0x00, 0xff};
				break;
			}
		}
	}
