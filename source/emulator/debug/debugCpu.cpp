...
DebugCpu::DebugCpu(IMemory* memory, std::shared_ptr<IRenderer> renderer)
	: Cpu(memory), _decompiler(memory), _cpu_recorder(), _cpu_renderer(renderer.get())
{
...
void DebugCpu::setDebugRenderer(IRenderer* renderer)
{
	this->_cpu_renderer.setRenderer(renderer);
}
