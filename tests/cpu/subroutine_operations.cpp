...
SCENARIO("Can perform brk operation", "[cpu][subroutine_ops]")
{
	...
	GIVEN("operation is brk [implicit]")
	{
			...
			"stack register value is 0xf3")
		{
			...
			cpu.status_register(0b1000'0011);
			...
			WHEN("operation is performed (7 cycles)")
			{
				...
				THEN(
					...
					"previous status register value + b flag (0b1001'0011) is on the stack")
				{
					REQUIRE(cpu.stack_register() == 0xf0);
					REQUIRE(cpu.program_counter() == 0x1215);
					REQUIRE(cpu.i_flag());
					...
					REQUIRE(mem._data[0x01f1] == 0b1001'0011);
				}
			}
		}
	}
			...
			"0b10000011, 0x02, 0x00 is stored on the stack")
		{
			...
			WHEN("operation is performed (6 cycles)")
			{
					...
					"program counter has a value of 0x0002")
				{
					...
					REQUIRE(cpu.status_register() == 0b1000'0011);
...
SCENARIO("Can jump to subroutine and then return", "[cpu][subroutine_ops]")
{
	SlowMemory mem;
	Cpu cpu(&mem);
	cpu.program_counter(0x0000);
	...
	GIVEN("operation is jsr [absolute]")
	{
		...
		AND_GIVEN("address value is 0x1215 and stack register value is 0xf2")
		{
			...
			AND_GIVEN("address 0x1215 contains a rts command")
			{
				mem._data[0x1215] = 0x60;

				WHEN("operation is performed (6 cycles + 6 cycles)")
				{
					cpu.cycle(6 + 6);
...
SCENARIO("Can run interrupt and then return", "[cpu][subroutine_ops]")
{
	SlowMemory mem;
	Cpu cpu(&mem);
	cpu.program_counter(0x0000);
	...
	GIVEN("operation is brk [implicit]")
	{
			...
			"stack register value is 0xf3")
		{
			...
			AND_GIVEN("address 0x1215 contains a rti command")
			{
				mem._data[0x1215] = 0x40;

				WHEN("operation is performed (7 cycles + 6 cycles)")
				{
					cpu.cycle(7 + 6);

					THEN(
						"stack register has a value of 0xf3, "
						"status register value is 0x0b1000'0011, and "
						"program counter has a value of 0x0002")
					{
						REQUIRE(cpu.stack_register() == 0xf3);
						REQUIRE(cpu.status_register() == 0b1001'0011);
						REQUIRE(cpu.program_counter() == 0x0002);
					}
				}
			}
